...
def cross_validation_thresh_log_reg(y,x, k_fold, threshs,seed=1):
    ...
    for i, thresh in enumerate(threshs) : 
        ...
        for k in range(k_fold):
            ...
            w, loss = logistic_regression(y_train,x_train,initial_w,max_iters=2000,gamma = 0.1)
        ...
        print('for thresh {t} average train accuracy {at} and average test acc {ate} \n false positives {fp} false negative {fn}'.format(t=thresh,at=acc_train,ate=acc_test,fp=fp,fn=fn))

    
    
    index_max_acc = np.argmax(accs_test) #return the index correspond to the best accuracy
    best_test_acc = accs_test[index_max_acc]
    best_train_acc = accs_train[index_max_acc]
    best_thresh = threshs[index_max_acc]

    print('cross validation on thresh , best acc_test {ate} , train_acc {at} corresponding to thresh {t} '.format(ate=best_test_acc,at=best_train_acc,t=best_thresh))
...
def cross_validation_thresh_least_sq(y,x, k_fold, threshs,seed=1):
    ...
    for i, thresh in enumerate(threshs) : 
        ...
        accs_test.append(acc_test) 
        accs_train.append(acc_train)
        print('for thresh {t} average train accuracy {at} and average test acc {ate}\n false positives {fp} false negatives {fn}'.format(t=thresh,at=acc_train,ate=acc_test,fp=fp, fn=fn))
...
def cross_validation_degree_lambda_log_reg(y,x, k_fold, lambdas,seed=1):
    ...
    for i, lambda_ in enumerate(lambdas) : 
        ...
        for k in range(k_fold):
            ...
            w, loss = reg_logistic_regression(y_train,x_train,lambda_,initial_w,max_iters=2000,gamma = 0.1)
            print(loss)
        ...
        accs_test.append(acc_test) 
        accs_train.append(acc_train)
        print('for lambda {t} average train accuracy {at} and average test acc {ate}\n false postives {fp} false negatives {fn}'.format(t=lambda_,at=acc_train,ate=acc_test,fp=fp,fn=fn))
    ...
    print('cross validation on lambda , best acc_test {ate} , train_acc {at} corresponding to lambda {t} '.format(ate=best_test_acc,at=best_train_acc,t=best_lambda))
...
def cross_validation_degree_lambda_ridge(y,x, k_fold, pairs,seed=1):
    ...
    for degree, lambda_ in pairs :
        ...
        accs_test.append(acc_test) 
        accs_train.append(acc_train)
        print('for degree {d}and lambda {t} average train accuracy {at} and average test acc {ate}'.format(d=degree, t=lambda_,at=acc_train,ate=acc_test))
    ...
    print('cross validation on (degree, lambda) , best acc_test {ate} , train_acc {at} corresponding to degree  {d} and lambda {l} '.format(ate=best_test_acc,at=best_train_acc,d=best_degree,l=best_lambda))
...
def cross_validation_degree_least_sq(y,x, k_fold, degrees,seed=1):
    ...
    print('cross validation on degree , best acc_test {ate} , train_acc {at} corresponding to degree {t} '.format(ate=best_test_acc,at=best_train_acc,t=best_degree))
...
def cross_validation_degree_log(y,x, k_fold, degrees,seed=1):
    ...
    for i, degree in enumerate(degrees) : 
        ...
        for k in range(k_fold):
            ...
            initial_w = np.random.rand(x_train.shape[1],)
            w, loss = logistic_regression(y_train,x_train,initial_w,max_iters=3000,gamma = 0.1)
            print(loss)
            f = np.vectorize(lambda x : 0 if x<0.5 else 1)
            pred_train = f(sigmoid(x_train.dot(w)))
            pred_test = f(sigmoid(x_test.dot(w)))
        ...
        accs_test.append(acc_test) 
        accs_train.append(acc_train)
        print('for degree {t} average train accuracy {at} and average test acc {ate}\n false postives {fp} false negatives {fn}'.format(t=degree,at=acc_train,ate=acc_test,fp=fp,fn=fn))
